...
original_path = "NYU_GT/"
training_path = "NYU_Hazy/"
...
list_of_aod_aod_brisque=[]
list_of_aod_aod_psnr=[]
list_of_aod_aod_ssim=[]
list_of_aod_aod_psnr2=[]
...
for original_file in files:
    ...
    if original_file[3]=='I':
        if count == 5:
             break
        count+=1

        name_split=original_file[:-11]
    

        training_files=os.listdir(training_path)
        for training_file in training_files:
            training_name=training_file[:-9]
            
            if name_split==training_name:
                
                
                clear_img=cv2.imread(original_path+original_file)
                hazy_image=cv2.imread(training_path+training_file)
                cv2.imwrite("op/nyu-clear-"+str(count)+".png", clear_img)
                cv2.imwrite("op/nyu-hazy-"+str(count)+".png", hazy_image)
                ...
                cv2.imwrite("op/nyu-dcp-"+str(count)+".png", dcp_img*255)
                cv2.imwrite("op/nyu-erc-"+str(count)+".png", erc_img*255)
                ...
                cv2.imwrite("op/nyu-aod-"+str(count)+".png", res*255)
                ...
                cv2.imwrite("op/nyu-dcp_aod-"+str(count)+".png", dcp_img*255)
                cv2.imwrite("op/nyu-erc_aod-"+str(count)+".png", erc_img*255)
                ...
                cv2.imwrite("op/nyu-aod_dcp-"+str(count)+".png", dcp_img*255)
                cv2.imwrite("op/nyu-aod_erc-"+str(count)+".png", erc_img*255)
                ...
                cv2.imwrite("op/nyu-erc_dcp-"+str(count)+".png", dcp_img*255)
                cv2.imwrite("op/nyu-dcp_erc-"+str(count)+".png", erc_img*255)
                ...
                # cv2.imshow("aodnet",res)
                cv2.imwrite("op/nyu-aod_aod-"+str(count)+".png", res*255)
                
                data = 255*res
                res=data.astype(np.uint8)

                list_of_aod_aod_ssim.append(evaluation.evaluate(res,clear_img,1))
                list_of_aod_aod_psnr.append(evaluation.evaluate(res,clear_img,2))
                list_of_aod_aod_brisque.append(evaluation.evaluate(res,clear_img,3))
                list_of_aod_aod_psnr2.append(evaluation.evaluate(res,clear_img,4))
...
print("aod_aod=","psnr ",np.mean(list_of_aod_aod_psnr),"ssim ",np.mean(list_of_aod_aod_ssim),"brisque ",np.mean(list_of_aod_aod_brisque),"psnr2",np.mean(list_of_aod_aod_psnr2))
